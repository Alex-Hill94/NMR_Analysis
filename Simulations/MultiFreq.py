...
for field in freqs:
    ...
    ydata_comb = (amps[0] * scale_glc) + (amps[1] * scale_lac) + (amps[2] * scale_cit)
    ...
    my_axs = [axs[0], axs[1], axs[2], axs[3]]
    for i in range(1, len(my_axs)):
        my_axs[i].plot(ppms[i-1], amps[i-1] * scales[i-1], color = colours[i-1], lw = linewidth)

    my_axs[0].plot(ppms[0], ydata_comb, color = 'darkslategrey', lw = linewidth)

    for my_ax in my_axs:
        my_ax.set_xlim([0.1, 5.6])
    ...
    axs[0].legend(custom_line, ['Combined Sim.'], frameon = False, loc = 'upper right', labelcolor = 'darkslategrey')
    axs[1].legend(custom_line, ['Glucose Sim.'], frameon = False, loc = 'upper right', labelcolor = colours[0])
    axs[2].legend(custom_line, ['Lactate Sim.'], frameon = False, loc = 'upper right', labelcolor = colours[1])
    axs[3].legend(custom_line, ['Citrate Sim.'], frameon = False, loc = 'upper right', labelcolor = colours[2])
    #axs[1].set_ylabel('$I(\delta)$')

    axs[1].set_ylabel('Intensity')
    axs[2].set_ylabel('Intensity')
    axs[3].set_ylabel('Intensity')
    axs[0].set_ylabel('Physiological Intensity')

    axs[3].set_xlabel('$\delta$ [ppm]')

    axs[0].set_title('%sMHz Simulation' % field)

    plt.subplots_adjust(hspace = 0.)
    plt.savefig('%sMHz_Plot.png' % field)
    plt.close()