...
root_path = '/Users/alexhill/Documents/UOL/Research/Companies/ViBo/Metabolomics/Data_Analysis/80MHzCitrateWET_Feb25/'

subdir = 'POS'
#filename_base = '2024-10-28-AH-Cit-01-10-zg30.txt'
...
for i in range(0, len(positions)):
    ...
    for j in range(0, len(wet_exps)):
        w_e = wet_exps[j]
        filename = '2025-02-07-AH-Cit-K_WET_f-'+pos[i]+'-'+str(w_e)+'.txt'
        print(filename)
...
for conc in concs:
    amp_a = u['Citrate/80MHz/'+conc+'/wet/amp_a'][()]
    amp_b = u['Citrate/80MHz/'+conc+'/wet/amp_b'][()]
    amp_c = u['Citrate/80MHz/'+conc+'/wet/amp_c'][()]
    n_scans = u['Citrate/80MHz/'+conc+'/wet/n_scans'][()]
    ppm_a = u['Citrate/80MHz/'+conc+'/wet/ppm_a'][()]
    ppm_b = u['Citrate/80MHz/'+conc+'/wet/ppm_b'][()]
    ppm_c = u['Citrate/80MHz/'+conc+'/wet/ppm_c'][()]

    f.create_dataset('Citrate/80MHz/'+conc+'/wet/amp_a', data = amp_a)
    f.create_dataset('Citrate/80MHz/'+conc+'/wet/amp_b', data = amp_b)
    f.create_dataset('Citrate/80MHz/'+conc+'/wet/amp_c', data = amp_c)
    f.create_dataset('Citrate/80MHz/'+conc+'/wet/n_scans', data = n_scans)
    f.create_dataset('Citrate/80MHz/'+conc+'/wet/ppm_a', data = ppm_a)
    f.create_dataset('Citrate/80MHz/'+conc+'/wet/ppm_b', data = ppm_b)
    f.create_dataset('Citrate/80MHz/'+conc+'/wet/ppm_c', data = ppm_c)
