...
root_path = '/Users/alexhill/Documents/UOL/Research/Companies/ViBo/Metabolomics/Data_Analysis/80MHzLactateWET_Feb25/'
...
for i in range(0, len(positions)):
    ...
    for j in range(0, len(wet_exps)):
        w_e = wet_exps[j]
        filename = '2025-02-06-AH-LAC-K_WET_f-'+pos[i]+'-'+str(w_e)+'.txt'
        print(filename)
...
for conc in concs:
    amp_a = u['Lactate/80MHz/'+conc+'/wet/amp_a'][()]
    amp_b = u['Lactate/80MHz/'+conc+'/wet/amp_b'][()]
    amp_c = u['Lactate/80MHz/'+conc+'/wet/amp_c'][()]
    n_scans = u['Lactate/80MHz/'+conc+'/wet/n_scans'][()]
    ppm_a = u['Lactate/80MHz/'+conc+'/wet/ppm_a'][()]
    ppm_b = u['Lactate/80MHz/'+conc+'/wet/ppm_b'][()]
    ppm_c = u['Lactate/80MHz/'+conc+'/wet/ppm_c'][()]
