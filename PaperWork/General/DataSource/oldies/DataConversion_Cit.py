...
root_path = '/Users/alexhill/Documents/UOL/Research/Companies/ViBo/Metabolomics/Data_Analysis/80MHzCitrate_Nov24/'
...
for i in range(0, len(positions)):
    ...
    for j in range(0, len(zg30_exps)):
        z3_e = zg30_exps[j]
        filename = '2024-11-28-AH-CIT-'+pos[i]+'_'+str(z3_e)+'_zg30.txt'
        print(filename)
        x, y = getData(path, filename)
        zg30_temp.append([x, y])
    for j in range(0, len(zg_exps)):
        z_e = zg_exps[j]
        filename = '2024-11-28-AH-CIT-'+pos[i]+'_'+str(z_e)+'_zg.txt'
        print(filename)
        x, y = getData(path, filename)
        zg_temp.append([x, y])
...
for conc in concs:
    amp_a = u['Citrate/80MHz/'+conc+'/zg30/amp_a'][()]
    amp_b = u['Citrate/80MHz/'+conc+'/zg30/amp_b'][()]
    amp_c = u['Citrate/80MHz/'+conc+'/zg30/amp_c'][()]
    n_scans = u['Citrate/80MHz/'+conc+'/zg30/n_scans'][()]
    ppm_a = u['Citrate/80MHz/'+conc+'/zg30/ppm_a'][()]
    ppm_b = u['Citrate/80MHz/'+conc+'/zg30/ppm_b'][()]
    ppm_c = u['Citrate/80MHz/'+conc+'/zg30/ppm_c'][()]
