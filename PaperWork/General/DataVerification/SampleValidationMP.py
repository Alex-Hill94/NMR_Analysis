...
substances = ['Lactate', 'Glucose', 'Citrate']

for i_s in range(0, len(substances)):
    substance = substances[i_s]
    ...
    sc_val = x_vals[-1]
    #x_vals = [1., 2., 3., 4.]
    scale = (sc_val/they a.mean(SIGS_0s[-3:]))
    ...
    axs.errorbar(unique_x, means, yerr=sems, fmt='o', label='Mean ± SEM', capsize=5, color=COLs[i_s])
    axs.plot(x_fit, y_fit, label=f'Fitted Line: y = {slope:.2f}x + {intercept:.2f}\n R2 = {R2:.5f}', linewidth=1.5, ls = LSs[i_s], alpha = 0.8, color = COLs[i_s])
    #axs[0].scatter(x, y, alpha=0.5, label='Original Data', color=COLs[i_s])
    DATA.append([slope, intercept, R2])
    # Customize the plot
    #axs.set_title(substance)
    #axs[i_s].legend()
...
axs.grid(True)
axs.set_xlabel('Relative Concentration')
axs.set_ylabel('Relative Intensity')
slope, intercept, R2 = DATA[0][0], DATA[0][1], DATA[0][2]
lac_lab = f'Lactate Fit: y = {slope:.2f}x + {intercept:.2f}\n $R^2$ = {R2:.5f}'
slope, intercept, R2 = DATA[1][0], DATA[1][1], DATA[1][2]
glc_lab = f'Glucose Fit: y = {slope:.2f}x + {intercept:.2f}\n $R^2$ = {R2:.5f}'
slope, intercept, R2 = DATA[2][0], DATA[2][1], DATA[2][2]
cit_lab = f'Citrate Fit: y = {slope:.2f}x + {intercept:.2f}\n $R^2$ = {R2:.5f}'

custom_lines = [Line2D([0], [0], marker = 'o', color='k', linestyle=None),
                Line2D([0], [0], color=COLs[0], linestyle=LSs[0]),
                Line2D([0], [0], color=COLs[1], linestyle=LSs[1]),
                Line2D([0], [0], color=COLs[2], linestyle=LSs[2])]
plt.legend(custom_lines, ['Mean ± SEM', lac_lab, glc_lab, cit_lab], fontsize = 12)
plt.savefig('sample_validation.pdf', bbox_inches = 'tight', pad_inches = 0.05)
#plt.show()
plt.close()
