...
plt.show()
    if method == 'gpv':   
        ...
        fwhm = 1
        eta = 0.
        initial_guess = [amp, cent, fwhm, eta]  # Initial guess for [amplitude, center, fwhm, eta]
        popt_gpv, pcov = curve_fit(gpv, x, y, p0=initial_guess)
        output = gpv(x, popt_gpv[0], popt_gpv[1], popt_gpv[2], popt_gpv[3])
'''
...
x_D22, y_D22 = grab_data(path = path, file_name = fnm)
_, y_D22_lor = fit_water(x_D22, y_D22, plot = True)
D22_fit = y_D22 - y_D22_lor

sample = 'D23'
nscan  = 256 
fnm = "%s_%s_ns%s.txt" % (sample, pulse, nscan)
x_D23, y_D23 = grab_data(path = path, file_name = fnm)
_, y_D23_lor = fit_water(x_D23, y_D23, plot = False)
D23_fit = y_D23 - y_D23_lor


sample = 'D24'
nscan  = 256 
fnm = "%s_%s_ns%s.txt" % (sample, pulse, nscan)
x_D24, y_D24 = grab_data(path = path, file_name = fnm)
_, y_D24_lor = fit_water(x_D24, y_D24, plot = False)
D24_fit = y_D24 - y_D24_lor
